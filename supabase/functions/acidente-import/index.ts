...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (agentesErr) {
      ...
      return respond(agentesErr.message, { status: 500, headers: corsHeaders })
    }
    ...
    if (lesoesErr) {
      ...
      return respond(lesoesErr.message, { status: 500, headers: corsHeaders })
    }
    ...
    if (partesErr) {
      ...
      return respond(partesErr.message, { status: 500, headers: corsHeaders })
    }
        ...
        if (rangeStart && rangeEnd) {
          ...
          for (let i = 0; i < pessoasArray.length; i += chunkSize) {
            ...
            if (acidentesErr) {
              ...
              return respond(acidentesErr.message, { status: 500, headers: corsHeaders })
            }
    ...
    if (errorLines.length > 1) {
      ...
      const key = `${owner}/erros/acidente_erros_${crypto.randomUUID()}.csv`
