...
import { runRelatorioTrocaEpi, assertRelatorioTrocaEnv } from "./_shared/relatorioTrocaEpiCore.ts"
...
const SERVICE_NAME = "relatorio-troca-epi"
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const result = await runRelatorioTrocaEpi()
    ...
    console.error("[relatorio-troca-epi] error:", message, error?.stack)
    if (shouldLog) {
      await finalizeLog({
        errors_count: 1,
