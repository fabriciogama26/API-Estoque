...
import { runRelatorioEstoqueSemanal, assertRelatorioSemanalEnv } from "./_shared/relatorioEstoqueSemanalCore.ts"
...
const SERVICE_NAME = "relatorio-estoque-semanal"
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const result = await runRelatorioEstoqueSemanal()
    ...
    console.error("[relatorio-estoque-semanal] error:", message, error?.stack)
    if (shouldLog) {
      await finalizeLog({
        errors_count: 1,
