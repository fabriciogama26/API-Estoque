-- Garante que o historico de cadastro base nao seja bloqueado por RLS.
-- A funcao roda como security definer com row_security off.
...
begin
  ...
  old_base = coalesce(old_row, '{}'::jsonb)
    ...
    - 'criado_em' - 'account_owner_id' - 'created_by_user_name' - 'updated_by_user_name';
  new_base = coalesce(new_row, '{}'::jsonb)
    - 'updated_at' - 'updated_by_user_id' - 'created_at' - 'created_by_user_id'
    - 'criado_em' - 'account_owner_id' - 'created_by_user_name' - 'updated_by_user_name';

  if old_base is not distinct from new_base then
    return new;
  end if;
  ...
  actor = auth.uid();
  owner_id = coalesce(
    (new_row->>'account_owner_id')::uuid,
    (old_row->>'account_owner_id')::uuid,
    public.my_owner_id(),
    actor
  );
