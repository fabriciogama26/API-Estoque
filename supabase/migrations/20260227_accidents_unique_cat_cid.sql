-- Garante CAT e CID unicos por account_owner_id.
create unique index if not exists accidents_unique_cat_per_owner
  on public.accidents (account_owner_id, lower(btrim(cat_number)))
  where cat_number is not null and btrim(cat_number) <> '';

create unique index if not exists accidents_unique_cid_per_owner
  on public.accidents (account_owner_id, lower(btrim(cid_code)))
  where cid_code is not null and btrim(cid_code) <> '';

-- Atualiza RPCs para validar duplicidade de CAT/CID.
  ...
  if v_cid is not null then
    select exists(
      select 1
        from public.accidents a
       where a.account_owner_id = v_owner
         and lower(btrim(a.cid_code)) = lower(v_cid)
    ) into v_invalid;
    if coalesce(v_invalid, false) then
      raise exception 'acidente_cid_duplicate' using errcode = 'P0001';
    end if;
  end if;
