-- Orcamento anual de compras separado da reposicao atual.
-- Mantem escopo por account_owner_id e expõe o calculo via RPC security definer.

create table if not exists public.agg_consumo_material_mensal (
  ...
  quantidade_saida numeric(14, 2) not null default 0,
  valor_saida numeric(14, 2) not null default 0,
  quantidade_entrada numeric(14, 2) not null default 0,
  valor_entrada numeric(14, 2) not null default 0,
  preco_medio numeric(14, 2) not null default 0,
  updated_at timestamptz not null default now(),
  constraint agg_consumo_material_mensal_pkey primary key (id),
  constraint agg_consumo_material_mensal_owner_material_mes_uidx unique (account_owner_id, material_id, ano_mes)
);

create index if not exists agg_consumo_material_mensal_owner_mes_idx
  on public.agg_consumo_material_mensal (account_owner_id, ano_mes);

create index if not exists agg_consumo_material_mensal_material_idx
  on public.agg_consumo_material_mensal (material_id, ano_mes);

alter table if exists public.agg_consumo_material_mensal enable row level security;
alter table if exists public.agg_consumo_material_mensal force row level security;

drop policy if exists agg_consumo_material_mensal_select_owner on public.agg_consumo_material_mensal;
create policy agg_consumo_material_mensal_select_owner
  on public.agg_consumo_material_mensal
  for select
  to authenticated
  using (
...
drop policy if exists agg_consumo_material_mensal_insert_owner on public.agg_consumo_material_mensal;
create policy agg_consumo_material_mensal_insert_owner
  on public.agg_consumo_material_mensal
  for insert
  to authenticated
  with check (
...
drop policy if exists agg_consumo_material_mensal_update_owner on public.agg_consumo_material_mensal;
create policy agg_consumo_material_mensal_update_owner
  on public.agg_consumo_material_mensal
  for update
  to authenticated
  using (
...
begin
  ...
  with movimentos as (
    select
      s.account_owner_id,
      nullif(s."materialId"::text, '')::uuid as material_id,
    ...
    join public.materiais m on m.id::text = nullif(s."materialId"::text, '') and m.account_owner_id = s.account_owner_id
    left join public.status_saida ss on ss.id::text = nullif(s.status::text, '')
    where s.account_owner_id = p_owner_id
      and nullif(s."materialId"::text, '') ~* '^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$'
    ...
    group by s.account_owner_id, nullif(s."materialId"::text, '')::uuid, date_trunc('month', s."dataEntrega")::date

    union all

    select
      e.account_owner_id,
      nullif(e."materialId"::text, '')::uuid as material_id,
    ...
    join public.materiais m on m.id::text = nullif(e."materialId"::text, '') and m.account_owner_id = e.account_owner_id
    left join public.status_entrada se on se.id::text = nullif(e.status::text, '')
    where e.account_owner_id = p_owner_id
      and nullif(e."materialId"::text, '') ~* '^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$'
    ...
    group by e.account_owner_id, nullif(e."materialId"::text, '')::uuid, date_trunc('month', e."dataEntrada")::date
  ),
  agrupado as (
    select
      account_owner_id,
...
begin
  ...
  perform public.rpc_refresh_consumo_material_mensal(p_owner_id, v_periodo_base_inicio, v_periodo_base_fim);

  return (
    with entradas_saldo as (
      select
        nullif(e."materialId"::text, '') as material_id,
      ...
      where e.account_owner_id = p_owner_id
        and nullif(e."materialId"::text, '') ~* '^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$'
        and coalesce(lower(se.status), lower(coalesce(e.status::text, ''))) <> 'cancelado'
      group by nullif(e."materialId"::text, '')
    ),
    saidas_saldo as (
      select
        nullif(s."materialId"::text, '') as material_id,
      ...
      where s.account_owner_id = p_owner_id
        and nullif(s."materialId"::text, '') ~* '^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$'
        and coalesce(lower(ss.status), lower(coalesce(s.status::text, ''))) <> 'cancelado'
      group by nullif(s."materialId"::text, '')
    ),
    consumo_base as (
      select
    ...
    materiais_base as (
      select
        ...
        m.nome,
        coalesce(m.fabricante, '') as fabricante,
    ...
    pesos as (
      ...
      left join public.f_previsao_gasto_mensal fp
        on fp.account_owner_id = p_owner_id
    ...
    select jsonb_build_object(
      ...
      'materiais_impacto', jsonb_build_object(
        'top_valor', coalesce((
          ...
          from (
            select material_id, nome, fabricante, grupo_material, consumo_previsto_qtd, estoque_utilizavel_qtd, pedidos_abertos_qtd, quantidade_necessaria_qtd, preco_atual, reajuste_valor, round((valor_com_reajuste * (1 + v_contingencia))::numeric, 2) as verba_recomendada
            from financeiro
        ...
        'top_risco', coalesce((
          ...
          from (
            select material_id, nome, fabricante, grupo_material, cobertura_dias, consumo_previsto_qtd, estoque_utilizavel_qtd, quantidade_necessaria_qtd, preco_atual, round((valor_com_reajuste * (1 + v_contingencia))::numeric, 2) as verba_recomendada
            from financeiro
            where quantidade_necessaria_qtd > 0
        ...
        'top_reajuste', coalesce((
          ...
          from (
            select material_id, nome, fabricante, grupo_material, reajuste_material, reajuste_valor, quantidade_necessaria_qtd, preco_atual, round((valor_com_reajuste * (1 + v_contingencia))::numeric, 2) as verba_recomendada
            from financeiro
        ...
        'reducao_orcamento', coalesce((
          ...
          from (
            select material_id, nome, fabricante, grupo_material, estoque_utilizavel_qtd, consumo_previsto_qtd, potencial_reducao_qtd, potencial_reducao_valor, preco_atual
            from financeiro
...
revoke all on function public.rpc_refresh_consumo_material_mensal(uuid, date, date) from public;
grant execute on function public.rpc_refresh_consumo_material_mensal(uuid, date, date) to authenticated, service_role;

revoke all on function public.rpc_orcamento_compra_12m_calcular(uuid, uuid, jsonb) from public;
grant execute on function public.rpc_orcamento_compra_12m_calcular(uuid, uuid, jsonb) to authenticated, service_role;
