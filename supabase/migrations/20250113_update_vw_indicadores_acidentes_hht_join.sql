-- Recria a view de indicadores de acidentes usando HHT mensal (hht_mensal) por centro/mes.
-- HHT não é mais lido de acidentes; TF/TG passam a usar hht_final agregado por centro/mês.
...
with acidentes_norm as (
  select
    ...
    lower(trim(coalesce(nullif(a.centro_servico, ''), 'Nao informado'))) as unidade_key,
  ...
  where a.data is not null
    ...
    and coalesce(a.cancel_motivo, '') is distinct from '__cancel_placeholder__'
),
hht_norm as (
  select
