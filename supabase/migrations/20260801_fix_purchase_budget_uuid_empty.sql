-- Corrige ambientes onde a primeira migration de orcamento anual ja foi aplicada
-- antes do tratamento de UUID vazio em entradas/saidas.
...
begin
  ...
  with movimentos as (
    ...
    left join public.status_saida ss
      on ss.id::text = nullif(s.status::text, '')
    where s.account_owner_id = p_owner_id
      ...
      and coalesce(lower(ss.status), lower(coalesce(nullif(s.status::text, ''), ''))) <> 'cancelado'
    ...
    left join public.status_entrada se
      on se.id::text = nullif(e.status::text, '')
    where e.account_owner_id = p_owner_id
      ...
      and coalesce(lower(se.status), lower(coalesce(nullif(e.status::text, ''), ''))) <> 'cancelado'
