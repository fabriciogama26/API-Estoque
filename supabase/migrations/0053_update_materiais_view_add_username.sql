-- Atualiza a view de materiais para expor username de cadastro/atualizacao.
DO $$
BEGIN
  ...
  SELECT
    ...
    m."usuarioCadastro",
    COALESCE(uc.display_name, uc.username) AS "usuarioCadastroNome",
    uc.username AS "usuarioCadastroUsername",
    m."usuarioAtualizacao",
    COALESCE(ua.display_name, ua.username) AS "usuarioAtualizacaoNome",
