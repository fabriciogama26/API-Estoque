-- Adiciona validacao de orcamento na aba Auditoria e corrige a previsao de demanda intermitente.
-- A regra antiga anualizava somente os meses com movimento e inflava itens eventuais.
-- Esta versao segmenta o consumo por padrao de demanda e expoe forecast bruto, travado e impacto.
  ...
  return (
    ...
    consumo_base as (
      select
        ...
        count(distinct a.ano_mes) filter (where coalesce(a.quantidade_saida, 0) > 0)::numeric as meses_com_movimento,
        sum(a.quantidade_saida) filter (where a.ano_mes >= (v_periodo_base_fim - interval '5 months')::date)::numeric as quantidade_saida_180d,
        sum(a.quantidade_saida) filter (where a.ano_mes >= (v_periodo_base_fim - interval '2 months')::date)::numeric as quantidade_saida_90d,
        sum(a.valor_saida) filter (where a.ano_mes >= (v_periodo_base_fim - interval '5 months')::date)::numeric as valor_saida_180d,
        sum(a.valor_saida) filter (where a.ano_mes >= (v_periodo_base_fim - interval '2 months')::date)::numeric as valor_saida_90d
    ...
    materiais_base as (
      select
        ...
        coalesce(cb.quantidade_saida_180d, 0)::numeric as quantidade_saida_180d,
        coalesce(cb.quantidade_saida_90d, 0)::numeric as quantidade_saida_90d,
        coalesce(cb.valor_saida_180d, 0)::numeric as valor_saida_180d,
        coalesce(cb.valor_saida_90d, 0)::numeric as valor_saida_90d,
        coalesce(cb.meses_com_movimento, 0)::numeric as meses_com_movimento,
        round((
          case
            when coalesce(cb.quantidade_saida, 0) > 0 then coalesce(cb.valor_saida, 0) / nullif(cb.quantidade_saida, 0)
            else coalesce(m."valorUnitario", 0)
          end
        )::numeric, 2) as preco_historico
    ...
    calculado as (
      select
        ...
        round((b.quantidade_saida_180d * 2)::numeric, 2) as saida_180d_anualizada_qtd,
        round((b.quantidade_saida_90d * 4)::numeric, 2) as saida_90d_anualizada_qtd,
        case
          when b.quantidade_saida_base <= 0 or b.meses_com_movimento <= 0 then 'sem_historico'
          when b.meses_com_movimento <= 2 then 'intermitente'
          when b.meses_com_movimento <= 5 then 'irregular'
          else 'recorrente'
        end as padrao_demanda,
        round((
          case
            when b.quantidade_saida_base <= 0 or b.meses_com_movimento <= 0 then 0
            when b.meses_com_movimento <= 2 then b.quantidade_saida_base * least(greatest(1 + v_crescimento, 0.80), 1.20)
            else (
              (0.50 * b.quantidade_saida_base)
              + (0.30 * (b.quantidade_saida_180d * 2))
              + (0.20 * (b.quantidade_saida_90d * 4))
            ) * (1 + v_crescimento)
          end
        )::numeric, 2) as previsao_bruta_12m_qtd,
        round((
          case
            when b.quantidade_saida_base <= 0 or b.meses_com_movimento <= 0 then 0
            when b.meses_com_movimento <= 2 then b.quantidade_saida_base * 1.20
            when b.meses_com_movimento <= 5 then b.quantidade_saida_base * 1.50
            else b.quantidade_saida_base * 2.00
          end
        )::numeric, 2) as limite_aplicado_12m_qtd
      from materiais_base b
    ),
    forecastado as (
      select
        c.*,
        round((c.previsao_bruta_12m_qtd * (v_horizonte::numeric / 12.0))::numeric, 2) as forecast_sem_trava,
        round((least(c.previsao_bruta_12m_qtd, c.limite_aplicado_12m_qtd) * (v_horizonte::numeric / 12.0))::numeric, 2) as forecast_com_trava,
        round((least(c.previsao_bruta_12m_qtd, c.limite_aplicado_12m_qtd) * (v_horizonte::numeric / 12.0))::numeric, 2) as consumo_previsto_qtd,
        round((greatest(c.previsao_bruta_12m_qtd - c.limite_aplicado_12m_qtd, 0) * (v_horizonte::numeric / 12.0))::numeric, 2) as forecast_reduzido_pela_trava_qtd,
        case
          when c.quantidade_saida_base > 0 then round((least(c.previsao_bruta_12m_qtd, c.limite_aplicado_12m_qtd) / c.quantidade_saida_base)::numeric, 2)
          else null
        end as fator_crescimento,
        case
          when c.quantidade_saida_base > 0 then round((c.previsao_bruta_12m_qtd / c.quantidade_saida_base)::numeric, 2)
          else null
        end as fator_crescimento_bruto,
        case
          when c.quantidade_saida_base <= 0 and c.demanda_extraordinaria_qtd > 0 then 'justificar'
          when c.quantidade_saida_base <= 0 then 'sem_historico'
          when (c.previsao_bruta_12m_qtd / nullif(c.quantidade_saida_base, 0)) <= 1.20 then 'normal'
          when (c.previsao_bruta_12m_qtd / nullif(c.quantidade_saida_base, 0)) <= 1.50 then 'atencao'
          when (c.previsao_bruta_12m_qtd / nullif(c.quantidade_saida_base, 0)) <= 2.00 then 'revisao'
          else 'justificar'
        end as status_validacao,
        case
          when c.quantidade_saida_base <= 0 or c.meses_com_movimento <= 0 then 'sem_historico'
          when c.meses_com_movimento <= 2 then 'intermitente_trava_120'
          when c.meses_com_movimento <= 5 then 'ponderado_50_30_20_trava_150'
          else 'ponderado_50_30_20_trava_200'
    ...
    enriquecido as (
      select
        f.*,
        round((f.consumo_previsto_qtd / greatest(v_horizonte, 1))::numeric, 2) as consumo_medio_mensal,
        round(((f.consumo_previsto_qtd / greatest(v_horizonte, 1)) * v_cobertura_final)::numeric, 2) as estoque_seguranca_qtd,
        case
          when f.consumo_previsto_qtd > 0 then round((f.estoque_atual / (f.consumo_previsto_qtd / greatest(v_horizonte, 1)) * 30)::numeric, 0)
          else null
        end as cobertura_dias,
        greatest(
          f.consumo_previsto_qtd
          + ((f.consumo_previsto_qtd / greatest(v_horizonte, 1)) * v_cobertura_final)
        ...
        greatest(
          f.estoque_utilizavel_qtd + f.pedidos_abertos_qtd
          - (f.consumo_previsto_qtd + ((f.consumo_previsto_qtd / greatest(v_horizonte, 1)) * v_cobertura_final)),
          0
    ...
    financeiro as (
      select
        e.*,
        round((e.preco_atual * (1 + e.reajuste_material))::numeric, 2) as preco_futuro,
        ...
        round((e.forecast_reduzido_pela_trava_qtd * e.preco_atual * (1 + e.reajuste_material))::numeric, 2) as valor_reduzido_pela_trava,
    ...
    validacao_rows as (
      select
        ...
        fabricante,
        grupo_material,
        quantidade_saida_base as realizado_12m,
        meses_com_movimento,
        padrao_demanda,
        quantidade_saida_180d as saida_180d,
        quantidade_saida_90d as saida_90d,
        saida_180d_anualizada_qtd,
        saida_90d_anualizada_qtd,
        previsao_bruta_12m_qtd as previsao_bruta,
        limite_aplicado_12m_qtd as limite_aplicado,
        consumo_previsto_qtd as previsao_final,
        fator_crescimento,
        fator_crescimento_bruto,
        estoque_atual,
        estoque_seguranca_qtd as estoque_final_desejado,
        quantidade_necessaria_qtd as necessidade_liquida,
        preco_historico,
        preco_futuro,
        round((valor_com_reajuste * (1 + v_contingencia))::numeric, 2) as impacto_financeiro,
        metodo_utilizado,
        status_validacao as status,
        forecast_sem_trava,
        forecast_com_trava,
        valor_reduzido_pela_trava,
        null::text as justificativa_manual,
        null::text as usuario_aprovador,
        round((greatest(consumo_previsto_qtd - quantidade_saida_base, 0) * preco_futuro)::numeric, 2) as aumento_quantidade_valor,
        round((quantidade_necessaria_qtd * greatest(preco_futuro - preco_historico, 0))::numeric, 2) as aumento_preco_valor,
        estoque_seguranca_valor,
        demanda_extraordinaria_qtd,
        (quantidade_saida_base <= 0 and estoque_atual < estoque_minimo and quantidade_necessaria_qtd > 0) as demanda_gerada_por_estoque_minimo
      from financeiro
    ),
    validacao_aumento_rank as (
      select
        vr.*,
        sum(vr.aumento_quantidade_valor) over (order by vr.aumento_quantidade_valor desc rows between unbounded preceding and current row) as aumento_quantidade_acumulado,
        sum(vr.aumento_quantidade_valor) over () as aumento_quantidade_total
      from validacao_rows vr
    ),
    validacao_resumo as (
      select
        count(*) filter (where status = 'justificar') as materiais_justificar,
        count(*) filter (where status = 'revisao') as materiais_revisao,
        count(*) filter (where status = 'atencao') as materiais_atencao,
        count(*) filter (where status = 'sem_historico') as materiais_sem_historico,
        count(*) filter (where fator_crescimento_bruto > 2) as materiais_acima_2x_bruto,
        count(*) filter (
          where aumento_quantidade_total > 0
            and aumento_quantidade_acumulado <= aumento_quantidade_total * 0.80
        ) as materiais_80_aumento,
        round(coalesce(sum(aumento_quantidade_valor), 0)::numeric, 2) as aumento_quantidade_valor,
        round(coalesce(sum(aumento_preco_valor), 0)::numeric, 2) as aumento_preco_valor,
        round(coalesce(sum(estoque_seguranca_valor), 0)::numeric, 2) as estoque_seguranca_valor,
        round(coalesce(sum(valor_reduzido_pela_trava), 0)::numeric, 2) as valor_reduzido_pela_trava
      from validacao_aumento_rank
    )
    select jsonb_build_object(
      ...
      'validacao_orcamento', jsonb_build_object(
        'resumo', jsonb_build_object(
          'materiais_80_aumento', vr.materiais_80_aumento,
          'materiais_justificar', vr.materiais_justificar,
          'materiais_revisao', vr.materiais_revisao,
          'materiais_atencao', vr.materiais_atencao,
          'materiais_sem_historico', vr.materiais_sem_historico,
          'materiais_acima_2x_bruto', vr.materiais_acima_2x_bruto,
          'aumento_quantidade_valor', vr.aumento_quantidade_valor,
          'aumento_preco_valor', vr.aumento_preco_valor,
          'estoque_seguranca_valor', vr.estoque_seguranca_valor,
          'contingencia_valor', rb.contingencia_valor,
          'valor_reduzido_pela_trava', vr.valor_reduzido_pela_trava
        ),
        'materiais', coalesce((
          select jsonb_agg(to_jsonb(q) order by q.status_ordem, q.fator_crescimento_bruto desc nulls last, q.impacto_financeiro desc)
          from (
            select
              material_id,
              nome,
              ...
              usuario_aprovador,
              aumento_quantidade_valor,
              aumento_preco_valor,
              demanda_gerada_por_estoque_minimo,
              case status
                when 'justificar' then 1
                when 'revisao' then 2
                when 'atencao' then 3
                when 'sem_historico' then 4
                else 5
              end as status_ordem
            from validacao_rows
            where impacto_financeiro > 0
               or fator_crescimento_bruto > 1.2
               or status in ('justificar', 'revisao', 'atencao', 'sem_historico')
            order by status_ordem, fator_crescimento_bruto desc nulls last, impacto_financeiro desc
            limit 250
          ) q
        ), '[]'::jsonb)
      ),
    ...
    from resumo_base rb
    cross join validacao_resumo vr
  );
end;
$$;
