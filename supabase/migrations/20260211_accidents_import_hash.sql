-- Adiciona hash de importacao para evitar duplicidade em importacoes.
alter table public.accidents
  add column if not exists import_hash text;

create unique index if not exists accidents_import_hash_uidx
  on public.accidents (account_owner_id, import_hash)
  where import_hash is not null and btrim(import_hash) <> '';
