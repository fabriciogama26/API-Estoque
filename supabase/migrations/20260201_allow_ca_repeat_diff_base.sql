-- Permite repetir C.A. em bases diferentes (mantem bloqueio para base igual).
...
BEGIN
  ...
  v_numero := NULLIF(fn_normalize_any(NEW."numeroEspecifico"), '');
  IF v_numero IS NULL THEN
    v_numero := NULLIF(fn_normalize_any(NEW."numeroCalcado"), '');
  END IF;

  IF v_numero IS NULL THEN
    v_numero := NULLIF(fn_normalize_any(NEW."numeroVestimenta"), '');
  END IF;
  ...
  IF v_ca_norm IS NOT NULL AND EXISTS (
    ...
    WHERE m.id <> NEW.id
      ...
      AND fn_normalize_any(m.fabricante) = fn_normalize_any(NEW.fabricante)
      AND fn_normalize_any(m."grupoMaterial") = fn_normalize_any(NEW."grupoMaterial")
      AND fn_normalize_any(m.nome) = fn_normalize_any(NEW.nome)
      AND COALESCE(
...
BEGIN
  ...
  SELECT
    -- Regra 1: CA duplicado na mesma base
    EXISTS (
      SELECT 1
      FROM base b
