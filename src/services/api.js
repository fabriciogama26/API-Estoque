  ...
  return registros
    .map((registro) => {
      ...
      return {
        ...
        dadosAntes: registro.dados_antes || registro.dadosAntes ? mapAsoRecord(registro.dados_antes ?? registro.dadosAntes) : null,
        dadosDepois: mapAsoRecord(registro.dados_depois ?? registro.dadosDepois ?? {}),
      }
    })
    .filter(Boolean)
}
...
export const api = {
  ...
  aso: {
    ...
    async downloadTemplate() {
      ...
      const headers = await buildAuthHeaders()
      const resp = await fetch(`${FUNCTIONS_URL}/aso-template`, { headers })
      if (!resp.ok) {
        throw new Error('Falha ao baixar modelo de ASO.')
      }
      const blob = await resp.blob()
      return { blob, filename: 'aso_template.xlsx' }
    },
    ...
    async importPlanilha(file) {
      try {
        ...
        const importsBucket = import.meta.env.VITE_IMPORTS_BUCKET || 'imports'
        const ownerId = await resolveImportsOwnerId()
        const path = `${ownerId}/aso/${(crypto?.randomUUID?.() ?? Date.now())}-${file.name}`
        ...
        const resp = await fetch(`${FUNCTIONS_URL}/aso-import`, {
          method: 'POST',
          headers,
          body: JSON.stringify({ path }),
        })
        ...
        const details = err?.details || {}
        reportClientError(
          'Falha ao importar ASO em massa.',
          err,
          {
            feature: 'aso',
            action: 'aso-import',
